...
# 		print(beta)
	...
	return decoder_loss + KLD + regressor_loss, KLD, regressor_loss, true_reg_loss
...
def model_train(loader, natural):
	...
	for batch_idx, (data, labels) in enumerate(loader):
		...
		loss, KLD, MSE, trl = loss_function(recon_batch, data, mu, logvar, cc, labels, natural)
...
def model_train_cross(loader):
	...
	for batch_idx, (data, labels) in enumerate(loader):
		...
		cc[labels == 0] = 0  # Sets the counted cells to 0 for unlabeled data, so that regressor_loss=0
		num_unlabeled += (labels == 0).sum()
		loss, KLD, MSE, trl = loss_function(recon_batch, data, mu, logvar, cc, labels, True)
...
def model_test(epo, natural):
	...
	with torch.no_grad():
		...
		for batch_idx, (data, labels) in enumerate(loader):
			...
			cc[labels == 0] = 0  # Sets the counted cells to 0 for unlabeled data, so that regressor_loss=0
			num_unlabeled += (labels == 0).sum()
			_, _, _, _ = loss_function(recon_batch, data, mu, logvar, cc, labels, natural)
...
def learn(start_epoch, max_epochs, plot_interval, test_interval, eval_interval, checkpoint_interval, delete_checkpoints):
	...
	for epoch in range(start_epoch, int(max_epochs) + 1):
		...
		print("Train Epoch: " + str(epoch) + "\tNAT: Dec: {:.3f}\tKLD: {:.4f}\tCor: {:.3f}\tMAE: {:.2f}\tDEV: {:.3f}\tRegr: {:.3f}\tSYN: Dec: {:.3f}\tKLD: {:.4f}\tCor: {:.3f}\tMAE: {:.2f}\tDEV: {:.3f}\tRegr: {:.3f}\ttime: {:.2f}s"
...
# 					g['lr'] = learning_rate
		...
		if checkpoint_path is not None:
			...
			if os.path.isfile(checkpoint_path):
				...
				if os.path.isfile("results/" + result_dir + "/logs/Decoder_nat_log.txt"):
					...
					print("Logs found and loaded")
				else:
					print("No logs found, creating new ones")
			else:
				print("No checkpoint found at '{}'".format(checkpoint_path))
				quit()
		else:
